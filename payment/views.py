...
def payment_process(request):
    ...
    merchant_upi_id = getattr(settings, 'MERCHANT_UPI_ID', 'stylesphere@upi')
    merchant_phone = getattr(settings, 'MERCHANT_PHONE_NUMBER', '+91 9781855165')
...
def webhook_handler(request):
    ...
    signature = request.META.get('HTTP_X_RAZORPAY_SIGNATURE', '').strip()

    if not webhook_secret:
        return JsonResponse({'status': 'error', 'message': 'Webhook secret is not configured on server.'}, status=500)

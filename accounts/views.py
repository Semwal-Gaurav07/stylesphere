...
def password_reset_view(request):
    ...
    if request.method == 'POST':
        ...
        # Enforce server-side state machine: prevent client from jumping directly to step 2 or 3
        if post_step == '3' and step != 3:
            messages.error(request, 'Unauthorized step access. Please verify your email and verification code first.')
            return redirect('accounts:password_reset')
        if post_step == '2' and step != 2:
            return redirect('accounts:password_reset')
